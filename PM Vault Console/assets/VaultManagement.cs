...
namespace PM_Vault_Console
{
    class VaultManagement
    {
        ...
        public void GeneratePwd(int length)
        {
            ...
            bool upper = finalString.Any(char.IsUpper);
            bool lower = finalString.Any(char.IsLower);
            bool number = finalString.Any(char.IsNumber);

            if (upper && lower && number) WriteLine($"Password: {finalString}");
            else goto GENERATE;
        }

        public void ReadList(string path, string password)
        {
            try
            {
        ...
        public void ReadAll(string path, string password)
        {
            try
            {   
                // load json and decrypt
